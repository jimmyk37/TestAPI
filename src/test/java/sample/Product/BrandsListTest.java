...
public class BrandsListTest {
	...
	public void testProductStructure() {
		Assert.assertTrue(response.jsonPath().getMap("products[0]").containsKey("id"), "Brand ID is missing");
		Assert.assertTrue(response.jsonPath().getMap("products[0]").containsKey("brand"), "Brand name is missing");
	}

	@Test
	public void testSpecificBrandDetails() {
		String brandName = response.jsonPath().getString("products.find { it.id == 3 }.brand");
